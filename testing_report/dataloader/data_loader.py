...
from .image_folder import make_dataset_by_one_image
from utils import task
import random


class CreateDataset(data.Dataset):
    def __init__(self, opt, img_file, mask_width):
        self.opt = opt
        self.mask_width = mask_width
        self.img_paths, self.img_size = make_dataset_by_one_image(img_file)
    ...
    def load_mask(self, img, index):
        ...
        if mask_type == 4:
            return task.center_bar_mask(img, self.mask_width)


def dataloader(opt, img_file, mask_width):
    datasets = CreateDataset(opt, img_file, mask_width)
