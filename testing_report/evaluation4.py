...
image_root = './groundtruths/non_360_2'
...
class Pluralistic_Inpainting_Model():
    ...
    def paint(self, img_file, save_path, nsampling):
        for c1, img in enumerate(image):
            save = []

            for c2, mask_width in enumerate([16, 32, 48, 64]):
                img_file = os.path.join(image_root, f"{c1+1}_{c2+1}.jpg")
                ...
                for i, data in enumerate(islice(dataset, self.opt.how_many)):
                    ...
                    save.extend(out)
            # embed()
            save_img_test(torch.cat(save), f"pretrained_task4_img{c1}.png")
