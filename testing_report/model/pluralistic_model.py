...
class Pluralistic(BaseModel):
    ...
    def __init__(self, opt):
        ...
        if self.isTrain:
            ...
            self.optimizer_G = torch.optim.Adam(itertools.chain(filter(lambda p: p.requires_grad, self.net_G.parameters()),
                        filter(lambda p: p.requires_grad, self.net_E.parameters())), lr=opt.lr, betas=(opt.beta1, 0.999))
    ...
    def test(self, save_path, nsampling):
        """Forward function used in test time"""
        # save the groundtruth and masked image
        self.save_results(self.img_truth, data_name='truth', save_path=save_path)
        self.save_results(self.img_m, data_name='mask', save_path=save_path)
        ...
        for i in range(nsampling):
            ...
            self.save_results(self.img_out, score=i, data_name='out', save_path=save_path)
            out.append(self.img_out)
        return out
