...
def random_bbox(config, batch_size):
    ...
    if config['mask_batch_same']:
        t = np.random.randint(margin_height, maxt) if maxt != 0 else 0
        # l = np.random.randint(margin_width, maxl) if maxl != 0 else 0
        l = img_width//2 + np.random.randint(-_range, _range)
